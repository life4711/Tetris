...
public class Block extends JPanel implements ActionListener,KeyListener//应该是继承JPanel
{
    ...
    static Button noStop = new Button("Cancle pause:");
    static Label scoreLab = new Label("Score:");
    static Label infoLab = new Label("Tip:");
    static Label speedLab = new Label("Lever:");
    ...
    static String butLab[] = {"Start","Restart","Low lever","High lever","Pause","Exit"};
    ...
    public void init()
    {
        ...
        add(infoTex);
        add(scoreLab);
        scoreLab.setBounds(320,15,70,20);
        scoreTex.setBounds(400,15,120,20);
        scoreTex.setBackground(Color.white);
        speedLab.setBounds(320,45,70,20);
        speedTex.setBounds(400,45,120,20);
        speedTex.setBackground(Color.white);

        but[1].setEnabled(false);
        but[4].setEnabled(false);

        infoLab.setBounds(320,75,70,20);
        infoTex.setBounds(400,75,120,20);
        infoTex.setBackground(Color.white);
        noStop.setBounds(400,360,120,25);
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        {
            infoTex.setText("Low lever!");
            speedMark--;
            if(speedMark <= 1)
            {
                speedMark = 1;
                infoTex.setText("The lowest lever!");
            }
            speedTex.setText(speedMark + "");
        }
        if(e.getSource() == but[3])//提高级数
        {
            infoTex.setText("High lever!");
            speedMark++;
            if(speedMark >= 9)
            {
                speedMark = 9;
                infoTex.setText("The highest lever" +
                        "!");
            }
            speedTex.setText(speedMark + "");
        }
